module Railsbox
  module Helpers
    ...
    #   default_config =  node["railsbox"]["default_config"]["passenger_runit"].to_hash
    ...
    def setup_unicorn_runit(app, app_dir)
      config = merge_runit_config(
        node["railsbox"]["default_config"]["unicorn_runit"],
        app["runit_config"]
      )
    ...
    def setup_nginx_site(app, app_dir, upstream_port)
      ...
      upstream_server = "#{hostname}:#{upstream_port}"
      config = merge_nginx_config(
        node["railsbox"]["default_config"]["nginx"],
        app["nginx_config"]
      )
    ...
    def setup_unicorn_config(app, app_dir, default_port)
      config = merge_unicorn_config(
        node["railsbox"]["default_config"]["unicorn"],
        app["unicorn_config"],
        app_dir,
        default_port
      )
